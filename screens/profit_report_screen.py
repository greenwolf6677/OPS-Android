"""
OPS Profit Report Screen
شاشة تقرير أرباح المنتجات
"""
...
class ProfitRecycleView(RecycleView):
    """عرض قائمة أرباح المنتجات"""
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.data = []
...
class ProfitReportScreen(Screen):
    ...
    # متغيرات الإحصائيات
    total_profit = StringProperty("₪ 0.00")
    total_sales = StringProperty("₪ 0.00")
    profit_margin = StringProperty("0%")
    ...
    def on_enter(self):
        """عند دخول الشاشة"""
        self.load_report()
        Logger.info("OPS Profit Report: Screen entered")
    ...
    def load_report(self):
        ...
        try:
            ...
            self.total_profit = f"₪ {total_profit_sum:.2f}"
            self.total_sales = f"₪ {total_sales_sum:.2f}"
            self.profit_margin = f"{((total_profit_sum / total_sales_sum) * 100) if total_sales_sum > 0 else 0:.1f}%"
            
            conn.close()
            Logger.info(f"OPS Profit Report: Loaded {len(data)} products")
            
        except Exception as e:
            Logger.error(f"OPS Profit Report: Error loading report - {e}")
